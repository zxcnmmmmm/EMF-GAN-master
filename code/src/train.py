...
# make_grid用于把几个图像按照网格排列的方式绘制出来，save_image用于保存图像
...
def parse_args():
    parser = argparse.ArgumentParser(description='EMF-GAN')
...
def main(args):
    ...
    args.model_save_file = osp.join(ROOT_PATH, 'save_models', str(args.CONFIG_NAME), stamp)
    ...
    print(train_dl, valid_dl ,train_ds, valid_ds, sampler)
    ...
    optimizerD = torch.optim.Adam(D_params, lr=0.0002, betas=(0.0, 0.9))
